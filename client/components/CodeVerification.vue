...
<template>
  <div class="codeverbox" v-on:keyup.enter="verifyCode">
    <div class="inner" v-show="isVerified">
      <h2
        style="margin-bottom: 16px; margin-top: 16px; color: #333; font-weight: 500;">
        ...
        <div style="width: 100%; padding: 10px 0;" v-if="!email">
          ...
          <div v-if="loading">
            <v-progress-circular indeterminate :size="32" :width="3"
                                 color="red darken-1"></v-progress-circular>
          </div>
          <!-- the user should not have to see and click this button, but it's there just in case -->
          <v-btn v-else outline @click="lookForAndSendCode">Send verification
            email
          </v-btn>
        </div>
        <div v-show="email">
          <p>We sent a temporary code to <strong>{{ email }}</strong> to
            make<span class="l-break"><br></span> sure you own it. Please enter
            it below:</p>
          <div class="input-container" v-on:keydown.delete="backspace"
               v-on:keydown.enter="verifyCode">
            <input id="box1" v-model="box1" ref="box1" maxlength="1"
                   @input="inputEntered"/>
            <input id="box2" v-model="box2" ref="box2" maxlength="1"
                   @input="inputEntered"/>
            <input id="box3" v-model="box3" ref="box3" maxlength="1"
                   @input="inputEntered"/>
            <input id="box4" v-model="box4" ref="box4" maxlength="1"
                   @input="inputEntered"/>
          </div>
          <p v-show="invalidCode" style="color: red;">Invalid code. Please try
            again.</p>
          <a @click="sendVerificationCode"
             style="color: gray !important; display: inline-block;">
            <i class="material-icons" style="margin-right: 3px;">refresh</i>Send
            a new code
          </a><br>
          <p v-show="loading" style="margin: 0; color: green">Email has been
            sent</p>
          <a @click="openChangeEmail"
             style="color: gray !important; margin-top: 4px; display: inline-block;">
            <i class="material-icons" style="margin-right: 3px;">edit</i>Edit
            email address
          </a>
        </div>
      </div>
      <div v-show="email && !isVerified " class="general-submit"
           @click="verifyCode">
        ...
        <p v-if="loading">
          <span style="padding: 0 4px;">
            <v-progress-circular indeterminate :size="16" :width="2"
                                 color="grey darken-1"></v-progress-circular>
          </span>
          Loading...
        </p>
